...
public class ManageDeathActions extends EldoCommand {
    ...
    public ManageDeathActions(Plugin plugin, Configuration configuration, ConversationRequester conversationRequester) {
        ...
        registerCommand("monster", new ManageMonsterDeathActions(plugin, configuration));
        registerCommand("player", new ManagePlayerDeathActions(plugin, configuration, conversationRequester));
    }
    ...
    private static class ManageMonsterDeathActions extends EldoCommand {
        private final Configuration configuration;
        ...
        public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
            ...
            if ("lightning".equalsIgnoreCase(field)) {
                ...
                Inventory inventory = Bukkit.createInventory(player, 18, "Manage Lightning Settings");

                InventoryActions actions = EldoUtilities.getInventoryActions().wrap(player, inventory, e -> {
                    configuration.save();
                    sendMobDeathActions(player, mobDeathActions);
                });
                ...
                actions.addAction(
                        new ActionItem(
                                ItemStackBuilder
                                        .of(Material.LEVER)
                                        .withDisplayName("Lightning active")
                ...
                actions.addAction(
                        new ActionItem(
                                ItemStackBuilder
                                        .of(Material.LEVER)
                                        .withDisplayName("Thunder active")
                ...
                actions.addAction(
                        new ActionItem(
                                ItemStackBuilder
                                        ...
                                        .withDisplayName("Lightning Probability")
                ...
                actions.addAction(
                        new ActionItem(
                                ItemStackBuilder
                                        ...
                                        .withDisplayName("Thunder Probability")
            ...
            if ("shockwave".equalsIgnoreCase(field)) {
                ...
                Inventory inventory = Bukkit.createInventory(player, 56, "Manage Shockwave Settings");
                ...
                actions.addAction(
                        ...
                        event -> {
                            ...
                            EldoUtilities.getDelayedActions().schedule(() -> {
                                Inventory effectInventory = Bukkit.createInventory(player, 56, localizer().getMessage("Configure Shockwave Effects"));
                ...
                actions.addAction(
                        ItemStackBuilder
                                .of(Material.CLOCK)
                                .withDisplayName("Min Effect Duration")
        ...
        private void sendMobDeathActions(Player player, MobDeathActions mobDeathActions) {

        }
    }

    private static class ManagePlayerDeathActions extends EldoCommand {
        private final Configuration configuration;
        private final ConversationRequester conversationRequester;

        public ManagePlayerDeathActions(Plugin plugin, Configuration configuration, ConversationRequester conversationRequester) {
            super(plugin);
            this.configuration = configuration;
            this.conversationRequester = conversationRequester;
        }
        ...
        public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
            ...
            if ("effects".equalsIgnoreCase(field)) {
                // TODO
                Inventory inventory = Bukkit.createInventory(player, 56, localizer().getMessage("Configure Respawn Effects"));
                InventoryActions actions = EldoUtilities.getInventoryActions().wrap(player, inventory,
                        e -> {
                            configuration.save();
                            sendPlayerDeathActions(playerDeathActions);
                        });
                ...
                for (String deathCommand : deathCommands) {
                    actions.addAction(
                            new ActionItem(ItemStackBuilder
                                    ...
                                    .withLore("§2Left Click to change", "§cRight Click to remove.")
                                    .build(),
                                    pos,
                                    e -> {
                                        switch (e.getClick()) {
                                            ...
                                            case SHIFT_LEFT:
                                                conversationRequester.requestInput(
                                                        player,
                                                        "Please enter command (Without /). Use {player} for player name.",
                                                        s -> true, 0, i -> {
            ...
            if ("addCommand".equalsIgnoreCase(field)) {
                if ("none".equalsIgnoreCase(field)) {
                    messageSender().send(MessageChannel.CHAT, MessageType.ERROR, sender, "Please enter a command.");
                    return true;
                }
                playerDeathActions.getDeathCommands().add(field);
                configuration.save();
                sendPlayerDeathActions(playerDeathActions);
                return true;
            }
        ...
        private void sendPlayerDeathActions(PlayerDeathActions playerDeathActions) {
