...
import de.eldoria.eldoutilities.utils.VectorUtil;
import org.bukkit.*;
...
public final class BloodMobUtil {
    private static final NamespacedKey IS_SPECIAL_MOB = ABloodNight.getNamespacedKey("isSpecialMob");
    private static final NamespacedKey IS_MOB_EXTENSION = ABloodNight.getNamespacedKey("isMobExtension");
    private static final NamespacedKey BASE_UUID = ABloodNight.getNamespacedKey("baseUUID");
    private static final NamespacedKey MOB_TYPE = ABloodNight.getNamespacedKey("mobType");

    private BloodMobUtil() {
    }
    ...
    public static void addPotionEffect(LivingEntity entity, PotionEffectType type, int amplifier, boolean visible) {
        addPotionEffect(entity, type, 60, amplifier, visible);
    }
     ...
     * @param visible   true if particles should be visible
     */
    public static void addPotionEffect(LivingEntity entity, PotionEffectType type, int seconds, int amplifier, boolean visible) {
        entity.addPotionEffect(new PotionEffect(type, seconds * 20, amplifier, visible, visible));
    }
    ...
    public static void dispatchShockwave(IShockwaveSettings settings, Location location) {
        ...
        for (Entity entity : getEntitiesAround(location, settings.getShockwaveRange())) {
            ...
            if (Math.abs(normalize.getY()) <= 0.1) {
                ABloodNight.logger().log(Level.FINE, "Adjusting shockwave direction. Vector is too flat.");
            ...
            ABloodNight.logger().log(Level.FINE, "Launching entity in direction " + normalize.toString()
    ...
    public static void dispatchLightning(ILightningSettings settings, Location location) {
